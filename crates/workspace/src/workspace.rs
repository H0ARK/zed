...
// Global configuration for default pane actions using function pointer
static DEFAULT_PANE_ACTION_FN: std::sync::OnceLock<fn() -> Box<dyn gpui::Action>> = std::sync::OnceLock::new();

pub fn set_default_pane_action_fn(action_fn: fn() -> Box<dyn gpui::Action>) {
    DEFAULT_PANE_ACTION_FN.set(action_fn).ok();
}

pub fn get_default_pane_action() -> Box<dyn gpui::Action> {
    DEFAULT_PANE_ACTION_FN
        .get()
        .map(|f| f())
        .unwrap_or_else(|| NewCenterTerminal.boxed_clone())
}
...
async fn join_channel_internal(
    ...
    'outer: loop {
        ...
        match status {
            ...
            | Status::Authenticated => continue,
            Status::Connected { .. } => break 'outer,
            Status::SignedOut => return Err(ErrorCode::SignedOut.into()),
            Status::AuthenticationError => return Err(ErrorCode::SignedOut.into()),
