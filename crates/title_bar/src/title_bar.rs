...
impl TitleBar {
    ...
    pub fn render_user_menu_button(&mut self, cx: &mut Context<Self>) -> impl Element {
        ...
        if let Some(user) = user_store.current_user() {
            ...
            let plan = self.user_store.read(cx).plan().filter(|_| {
                    ...
                    ContextMenu::build(window, cx, |menu, _, _cx| {
                        menu.link(
                            format!(
                                ...
                                match plan {
                                    None => "None",
                                    Some(cloud_llm_client::Plan::ZedFree) => "Zed Free",
                                    Some(cloud_llm_client::Plan::ZedPro) => "Zed Pro",
                                    Some(cloud_llm_client::Plan::ZedProTrial) => "Zed Pro (Trial)",
                                }
                            ),
